...
export default function SovleExamloged() {
  let { id } = useParams();
  ...
  const addnamehandler=(e)=>{
    e.preventDefault();
   if(localStorage.getItem(exam._id) || !exam.Questions)
    {
  ...
  useEffect(() => {
    ...
    setloading(false);
    Axios.get( 
      BACKEND_DOMAIN+`/api/exam/examsloged/${id}`,
      config
    ).then((response)=>{
      setloading(true);
  ...
  const sendanswershandler=(e)=>{
    ...
    setloading(false);
    Axios.post( 
      BACKEND_DOMAIN+'/api/exam/solveexamloged',
