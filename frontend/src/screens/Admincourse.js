...
export default function Admincourse() {
  ...
  const submithandler=(e)=>{
    ...
    Axios.post( 
      BACKEND_DOMAIN+'/api/course',
      {...inputs,Sections:qscodes,Private:isprivate.current.checked,Img:avatar},
      config
    ).then((response)=>{
      setloadingcode(true);
      const Toast = Swal.mixin({
  ...
  useEffect(() => {
    ...
    setloading(false);
    Axios.get( 
      BACKEND_DOMAIN+`/api/course?num=${num}&type=${selectinput.current.value}&s=${searchinput.current.value}`,
      config
    ).then((response)=>{
      setloading(true);
      setqs(response.data);
  ...
  const deletecode=async(_id)=>{
    ...
    setloading(false);
    Axios.post( 
      BACKEND_DOMAIN+`/api/course/delete`,
      inputs,
      config
    ).then((response)=>{
      setloading(true);
  ...
  const editcode=async(_id,Name,Sections,Private,Year,Term,Qimg)=>{
      setInputs2((prev)=>({
        _id:_id,
        Name:Name,
        Sections:Sections,
    ...
    const submithandler2=(e)=>{
      ...
      setloadingcode(false);
      Axios.post( 
        BACKEND_DOMAIN+'/api/course/edit',
        {...inputs2,Sections:qscodes,Private:privateedit.current.checked,Img:avatar.replace(BACKEND_DOMAIN, "")},
        config
      ).then((response)=>{
        setloadingcode(true);
        const Toast = Swal.mixin({
  ...
  useEffect(() => {
    ...
    setloading(false);
    Axios.get( 
      BACKEND_DOMAIN+`/api/section?num=${1}&type=${selectqinput.current.value}&s=${searchqinput.current.value}`,
      config
    ).then((response)=>{
      setloading(true);
      ...
      //edit ************
        ...
        useEffect(() => {
          ...
          setloading(false);
          Axios.get( 
            BACKEND_DOMAIN+`/api/section?num=${1}&type=${selectqinputedit.current.value}&s=${searchqinputedit.current.value}`,
            config
          ).then((response)=>{
            setloading(true);
  ...
  useEffect(() => {
    const config = {
      ...
      setloading(false);
      Axios.get( 
        BACKEND_DOMAIN+`/api/course/solveexam`,
        config
      ).then((response)=>{
        setloading(true);
        setsearchqsedit(response.data);
    ...
    <div className="admincont admin">
      ...
      <div className='search'>
        ...
        <div className='toright'>
          <button onClick={()=>{addpanel.current.style.display="block";overlay.current.style.display="block"}}>اضافة كورس</button>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                {qs.map((code,i)=>{
                  return(<tr>
                          ...
                          <td>
                            <i className="fa-solid fa-pen" onClick={()=>editcode(code._id,code.Name,code.Sections,code.Private,code.Year,code.Term,code.Img)}></i>
        ...
        <div className='add bigpanel' ref={addpanel}>
          ...
          <label>
            ...
            <select  name="Year" onChange={handleChange}>
              ...
              <option value="programing">مهارة البرمجة</option>
              <option value="graphic">مهارة الجرافيك</option>
              <option value="art">مهارة الرسم</option>
              <option value="montage">مهارة المونتاح</option>
            </select>
          </label>
          ...
          <label class="container-checkbox">is private
            ...
            <h1 style={{"textAlign":"center"}}>السكشنز</h1>
